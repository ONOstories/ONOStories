...
Deno.serve(async (req) => {
  ...
  const supabase = createServerClient(
    ...
    { cookies: {
        get: (n) => jar.get(n),
        set: (n, v, o) => { let c = `${n}=${v}; Path=${path}; HttpOnly; SameSite=${sameSite}`; if (secure) c += '; Secure'; if (o?.maxAge) c += `; Max-Age=${o.maxAge}`; setCookies.push(c); },
  ...
  return new Response(JSON.stringify({ story: data ?? null }), { headers, status: 200 });
});
